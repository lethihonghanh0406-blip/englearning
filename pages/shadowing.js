...
export default async function shadowingPage(app) {
  ...
  let ytPlayer       = null
  let ytReady        = false
  let layoutLessonId = 'INIT'   // tracks which lesson the layout was built for
  ...
  const { data } = await supabase.from('shadowing_lessons').select('*').order('created_at')
  lessons = data || []
  ...
  function getTimestamps() { return getLesson()?.sentences_timestamps || [] }
  function getVid(lesson) {
  ...
  function compareZH(expected, spoken) {
    const exp = splitZH(expected), spk = splitZH(spoken)
    return exp.map((c, i) => ({ char: c, ok: spk[i] === c }))
  }
  ...
  function startWaveform(stream) {
    ...
    function draw() {
      ...
      buf.forEach((v, i) => {
        ...
        g.addColorStop(0, '#ef4444'); g.addColorStop(1, '#fca5a5')
        ctx.fillStyle = g
  ...
  function stopWaveform() {
    ...
    const c = document.getElementById('sh-waveform')
  ...
  function startRecording() {
    ...
    updateCard()
    recognition = new SR()
    recognition.lang = 'zh-CN'; recognition.interimResults = true; recognition.maxAlternatives = 1
  ...
  function initYTPlayer(videoId) {
    ...
    ensureYTAPI().then(() => {
      const try_ = () => {
        const el = document.getElementById('sh-yt-container')
        if (!el) { setTimeout(try_, 100); return }
        if (ytPlayer) { ytPlayer.destroy(); ytPlayer = null }
        ytPlayer = new YT.Player('sh-yt-container', {
  ...
  function playOnYT() {
    ...
    // Use stored timestamp if available, otherwise estimate proportionally (Part1 ≈ 286s)
    const t = (ts[sentIdx] != null) ? ts[sentIdx] : Math.round(sentIdx * 286 / Math.max(total, 1))
    ytPlayer.seekTo(t, true)
    ytPlayer.playVideo()
  }

  function speakTTS() {
    const s = getSentences()[sentIdx]
    if (!s) return
  ...
  function render() {
    ...
    const vid    = getVid(lesson)
    if (selId !== layoutLessonId || !document.getElementById('sh-card')) {
      renderLayout(lesson, vid)
      layoutLessonId = selId
  ...
  function renderLayout(lesson, vid) {
    ...
    app.innerHTML = `
      <div style="min-height:100vh;background:#f8faff;display:flex;flex-direction:column">
        ...
        <div style="flex:1;display:flex;overflow:hidden;min-height:calc(100vh - 56px)">
          <div id="sh-sidebar" style="width:240px;min-width:240px;background:white;border-right:1px solid #e2e8f0;overflow-y:auto">
              ...
              ${vid ? `
                <div style="background:white;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden">
                  <div style="aspect-ratio:16/9">
                    <div id="sh-yt-container" style="width:100%;height:100%"></div>
                  </div>
                </div>` : ''}
              <div id="sh-card">${buildCard()}</div>
            </div>
          </div>
        </div>
      </div>`
  ...
  function updateSidebar() {
    ...
    const lt = { beginner:'#166534', intermediate:'#92400e', advanced:'#dc2626' }
    const ll = { beginner:'Cơ bản',  intermediate:'Trung cấp', advanced:'Nâng cao' }
    const el = document.getElementById('sh-sidebar')
    if (el) el.innerHTML = buildSidebarContent(lc, lt, ll)
  }

  function updateCard() {
    const el = document.getElementById('sh-card')
    if (el) el.innerHTML = buildCard()
  }
          ...
          : lessons.map(l => {
              ...
              return `<div onclick="shSelect('${l.id}')"
                ...
                <span style="font-size:11px;font-weight:600;padding:2px 8px;border-radius:10px;
                  background:${lc[lv]||'#f1f5f9'};color:${lt[lv]||'#374151'}">${ll[lv]||lv}</span>
              </div>`
            }).join('')}
      </div>`
  }
  ...
  function buildCard() {
    ...
    return `
      <div style="background:white;border-radius:14px;border:1px solid #e2e8f0;padding:20px">
          ...
          <div style="display:flex;gap:6px">
            <button onclick="shTogglePinyin()"
              style="padding:5px 12px;border-radius:7px;border:none;cursor:pointer;font-size:12px;font-weight:600;
                background:${showPinyin?'#2563eb':'#f1f5f9'};color:${showPinyin?'white':'#64748b'}">Phiên âm</button>
            ${viLines.length ? `
              <button onclick="shToggleVI()"
                style="padding:5px 12px;border-radius:7px;border:none;cursor:pointer;font-size:12px;font-weight:600;
                  background:${showVI?'#2563eb':'#f1f5f9'};color:${showVI?'white':'#64748b'}">Dịch</button>` : ''}
          </div>
        </div>
        ...
        <div style="min-height:52px;margin-bottom:12px">${sentenceHTML}</div>

        <!-- Audio buttons -->
        <div style="display:flex;gap:8px;margin-bottom:12px">
          <button onclick="shPlayYT()"
            style="padding:7px 16px;border-radius:8px;border:none;cursor:pointer;font-size:13px;font-weight:600;
              background:#eff6ff;color:#2563eb;display:flex;align-items:center;gap:5px">
            ▶ Nghe
          </button>
          <button onclick="shSpeak()"
            style="padding:7px 16px;border-radius:8px;border:1px solid #e2e8f0;cursor:pointer;font-size:13px;
              background:white;color:#64748b;display:flex;align-items:center;gap:5px">
            🔊 TTS
          </button>
        </div>
      ...
      <div style="display:flex;gap:10px">
        ...
        <button onclick="shNext()" ${sentIdx>=sentences.length-1?'disabled':''}
          style="flex:1;padding:11px;border-radius:10px;border:none;background:#fef9c3;
      ...
      ${lastResult ? `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div style="background:white;border-radius:12px;border:1px solid #e2e8f0;padding:16px 20px">
            ...
            <div style="font-size:32px;font-weight:700;color:${lastResult.score>=80?'#16a34a':lastResult.score>=60?'#d97706':'#dc2626'}">
          ...
          <div style="background:white;border-radius:12px;border:1px solid #e2e8f0;padding:16px 20px">
            ...
            <div style="font-size:32px;font-weight:700;color:${lastResult.accuracy>=80?'#16a34a':lastResult.accuracy>=60?'#d97706':'#dc2626'}">
  ...
  window.shTogglePinyin = () => { showPinyin = !showPinyin; updateCard() }
  window.shToggleVI     = () => { showVI = !showVI; updateCard() }
  window.shToggleRec    = () => toggleRecording()
  window.shPlayYT       = () => playOnYT()
  window.shSpeak        = () => speakTTS()
}
