...
export default async function chineseWritingPage(app) {
  ...
  async function startWrite() {
    ...
    const words = buildSession(); let idx = 0, mistakes = 0, done = false
    function show(i) {
      idx = i; mistakes = 0; done = false; const w = words[i]
      ...
      document.getElementById('cw-body').innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:2px">
          ...
          <button onclick="cwSpeak('${w.char}')"
            style="background:none;border:none;cursor:pointer;font-size:20px;color:#94a3b8;padding:4px;line-height:1">🔊</button>
        </div>
      ...
      try {
        writer = HanziWriter.create('cw-canvas', w.char, {
          width:300, height:300, padding:5, showOutline:true,
        ...
        writer.quiz({
          ...
          onComplete: () => {
            ...
            if (h) h.textContent = mistakes === 0 ? '✅ Hoàn hảo!' : `Xong! ${mistakes} lỗi.`
